...
def main(settings):
    ...
    # make output_dir
    output_dir = os.path.join('test_files', datetime.now().strftime('%Y-%m-%d_%H-%M-%S'))
    ...
    if settings['use_data_generators']:
        ...
        X_batch, y_batch = train_dataset.__getitem__(1)
        print(X_batch['drugA'].shape)
        X_batch, y_batch = train_dataset.__getitem__(2)
        print(X_batch['drugA'].shape)
    ...
    if 'opt_hyperparams_path' in settings:
        ...
        tuner.search(n_configurations=5, hyperparam_space=opt_hyperparams, random_seed=12321,
...
if __name__ == '__main__':
    # Parse settings file
    with open('test_files/test_keras_pipeline.yml', 'r') as stream:
        try:
